 ...
 */
inline unsigned int  var(int literal) {
	return abs(literal);
}
...
void parseInput() {
	...
	// Read clauses
	for (unsigned int  clause = 0; clause < numClauses; ++clause) {
		int literal;
...
void updateActivityForLiteral(int literal) {
	...
	unsigned int  index = var(literal);
	if (literal > 0) {
...
bool propagateGivesConflict() {
	while (indexOfNextLiteralToPropagate < modelStack.size()) {
		...
		for (unsigned int  i = 0; i < clausesToPropagate.size(); ++i) {
			...
			//traverse the clause
			for (unsigned int  k = 0; not isSomeLiteralTrue and k < clause.size(); ++k) {
